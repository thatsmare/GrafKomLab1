...
public:
	...
	baseDamage = 5; 
	texture = LoadTexture("asteroid_circle1.png");
	...
	void Draw() const override {
		...
		DrawTextureEx(texture, triPosition, transform.rotation, scale, WHITE);
	}
private:
	Texture2D texture;
	float scale = 0.25f;
...
public:
	...
	baseDamage = 10;
	texture = LoadTexture("asteroid_rectangle.png"); 
	SetTextureFilter(texture, TEXTURE_FILTER_BILINEAR);
	GenTextureMipmaps(&texture);
	}
	~SquareAsteroid() {
	...
	void Draw() const override {
		...
		DrawTextureEx(texture, SqPosition, transform.rotation, scale, WHITE);
	}
private:
	Texture2D texture;
	float scale = 0.25f;
...
public:
	...
	baseDamage = 15;
	texture = LoadTexture("supernova.png"); 
	SetTextureFilter(texture, TEXTURE_FILTER_BILINEAR);
	GenTextureMipmaps(&texture);
	}
	~PentagonAsteroid() {
	...
	void Draw() const override {
		...
		DrawTextureEx(texture, PenPosition, transform.rotation, scale, WHITE);
	}
private:
	Texture2D texture;
	float scale = 0.25f;
		...
		while (!WindowShouldClose()) {
			...
			// Shooting
			{
				...
				if (player->IsAlive() && IsKeyDown(KEY_M)) {
					...
					if (bottomShotTimer > maxInterval) {
						bottomShotTimer = fmodf(bottomShotTimer, maxInterval);
					}
				}
			}
...
private:
	...
	static constexpr float C_SPAWN_MIN = 1.0f;
	static constexpr float C_SPAWN_MAX = 4.0f;
